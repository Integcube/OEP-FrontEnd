...
import { ExcuteInsurenceFormComponent } from './dialogs/Excute-insurence-form.component';
import { APISaveData, OT_OutageStatus, OT_outageTracker, PreInsurenceFilterDto } from './Excute-insurence.model';

@Component({
  selector: 'app-Excute-insurence',
  templateUrl: './Excute-insurence.component.html',
  styleUrls: ['./Excute-insurence.component.sass']
})
export class ExcuteInsurenceComponent extends UnsubscribeOnDestroyAdapter implements OnInit, AfterViewInit {
  ...
  filterObj: PreInsurenceFilterDto = {
    ...
    Year: -1,
    status: -1,
    isTlsAdmin:this.user.isTlsAdmin
  }
  displayFilter: Boolean = false;

  constructor(private snackBar: MatSnackBar, private dataService: ExcuteInsurenceService, private dataService2: CommonService, public dialog: MatDialog,) { super() }
  displayedColumns: string[] = ['preInsuranceActionId','siteTitle', 'recommendationTitle', 'actionTitle','guidelines', 'yearId', 'status', 'deadline','actions'];
  ...
  viewAction(action: any) {
    const dialogRef = this.dialog.open(ExcuteInsurenceFormComponent, {
      width: '750px',
      data: {
        action: "view",
        outageAction: action
